...
If you run into a problem while migrating that was not covered by this migration guide, please open an issue on the [bevy-website](https://github.com/bevyengine/bevy-website) repo or file a PR directly with the correct migration advice.
...
PRs: [#21388](https://github.com/bevyengine/bevy/pull/21388), [#21435](https://github.com/bevyengine/bevy/pull/21435), [#21436](https://github.com/bevyengine/bevy/pull/21436), [#21437](https://github.com/bevyengine/bevy/pull/21437)
...
PRs: [#21562](https://github.com/bevyengine/bevy/pull/21562), [#21613](https://github.com/bevyengine/bevy/pull/21613)
...
PRs: [#21608](https://github.com/bevyengine/bevy/pull/21608), [#21817](https://github.com/bevyengine/bevy/pull/21817)

- `ScheduleGraph::topsort_graph` has been moved to `DiGraph::toposort`, and now takes a `Vec<N>` parameter for allocation reuse.
- `ReportCycles` was removed: instead, `DiGraphToposortError`s should be immediately wrapped into hierarchy graph or dependency graph `ScheduleBuildError` variants.
...
// Because `fails_validation` is invalid, trying to run this combinator system will return an error.
...
PRs: [#21470](https://github.com/bevyengine/bevy/pull/21470), [#21537](https://github.com/bevyengine/bevy/pull/21537)
...
PRs: [#19350](https://github.com/bevyengine/bevy/pull/19350), [#19433](https://github.com/bevyengine/bevy/pull/19433), [#19451](https://github.com/bevyengine/bevy/pull/19451)
...
In 0.17 the `Aabb` [was not updated](https://github.com/bevyengine/bevy/issues/4294) if the mesh or sprite was modified. This has been fixed in 0.18. If you were working around the issue by manually updating or removing the `Aabb`, then the workaround is no longer needed.
...
Previously `AssetServer`s `new` and `new_with_method_check` methods would take `AssetSources`. Now, these methods take `Arc<AssetSources>`.
...
PRs: [#19363](https://github.com/bevyengine/bevy/pull/19363), [#21792](https://github.com/bevyengine/bevy/pull/21792)
...
Matching both [hashbrown](https://github.com/rust-lang/hashbrown/pull/648) and the `std` library, we've renamed all the `get_many_*` methods on `bevy_platform::HashMap` to `get_disjoint_*`. So rename:
...
In 0.17 the conversion was applied to nodes and meshes within glTF scenes. This worked well for some users, but had [bugs](https://github.com/bevyengine/bevy/issues/20621) and didn't work well for other users. In particular, node conversion caused issues with cameras and lights.
